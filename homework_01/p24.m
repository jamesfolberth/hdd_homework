...
function [] = plot_minmax_fit(savefile,titlestring,figurefilename)
   ...
   [f_min,p_min] = leasqr(n,mineigs(:),[0;1],...
      @(x_,p_) fit_objective(x_,p_,n,N));
   [f_max,p_max] = leasqr(n,maxeigs(:),[0;-1],...
      @(x_,p_) fit_objective(x_,p_,n,N));
   ...
   plot(n_rep,p_min(2)*sqrt(n_rep+p_min(1)),'g');
   plot(n_rep,p_max(2)*sqrt(n_rep+p_min(1)),'g');
   hold off;
...
function [y] = fit_objective(x,p,n,N_sample)
   ...
   for i=1:numel(n)
      y(N_sample*(i-1)+1:N_sample*i) = p(2)*sqrt(x(i)+p(1));
   end
end
